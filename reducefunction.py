...
total_snowfall = reduce( lambda acc , element : acc +element['snow'] , weatherdata, 0 )
pprint.pp(total_snowfall)

# TODO: how much total precipitation is in the entire dataset?
total_preci = reduce( lambda acc , element : acc +element['snow'] + element['prcp'] , weatherdata, 0 )
pprint.pp(total_preci)
...
pprint.pp(f"The warmest day which had snowfall is {result['date']} temperature {result['tmax']} snowfall {result['snow']}")
